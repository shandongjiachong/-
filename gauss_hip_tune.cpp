...
GpuResult gauss_gpu_basic_tuned(int n, std::vector<float>& A, int threads) {
    int width = n + 1;
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        test_sizes = {1024, 2048};
    }

    std::vector<int> thread_options = {128, 256, 512};
    const int repeat = 3;

    warmup_gpu();

    std::cout << "===== HIP Gaussian Elimination Thread Tuning Test =====" << std::endl;
    std::cout << "device=" << prop.name << std::endl;
    std::cout << "repeat=" << repeat << std::endl;
    std::cout << "format: n threads trial serial_ms total_ms kernel_ms total_speedup kernel_speedup err" << std::endl;
    std::cout << std::fixed << std::setprecision(3);

    for (int n : test_sizes) {
        auto base = generate_matrix(n);

        double serial_ms = 0.0;
        {
            std::vector<float> A_serial = base;
            auto ts1 = std::chrono::high_resolution_clock::now();
            gauss_serial(n, A_serial.data());
            auto ts2 = std::chrono::high_resolution_clock::now();
            serial_ms = std::chrono::duration<double, std::milli>(ts2 - ts1).count();
        }

        for (int threads : thread_options) {
            for (int t = 1; t <= repeat; ++t) {
                std::vector<float> A_gpu = base;
                GpuResult res = gauss_gpu_basic_tuned(n, A_gpu, threads);
